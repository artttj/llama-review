---
description: Run parallel specialist code reviews through Ollama models. Each lane uses a model chosen for that domain's strengths. Groups changed files into review lanes, dispatches parallel reviews, and merges findings into one report.
mode: subagent
permission:
  edit: deny
  write: deny
...
Cloud and local models both dispatch via `ollama run`. Cloud models (with `:cloud` suffix) do NOT appear in `ollama list` — **do not run `ollama list` to check for cloud models.** Use `--local` to switch to local models.
